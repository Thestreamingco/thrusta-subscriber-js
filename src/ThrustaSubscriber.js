...
function ThrustaSubscriber(url, opt) {
    ...
    if (opt.shared) {
        if (!("localStorage" in global)) {
            throw "localStorage unavailable for use in shared ThrustaSubscriber";
        }

        var pre = "ThrustaSubscriber:" + this.url + ":shared:";
...
Emitter(ThrustaSubscriber.prototype);
...
var defaultTransportOptions = function () {
    ...
    for (var n in ThrustaSubscriber.prototype.SubscriberClass) {
        if (n != "__slave") {
            opt[n] = {};
        }
    }
...
ThrustaSubscriber.prototype.initializeTransport = function (possibleTransports) {
...
ThrustaSubscriber.prototype.start = function () {
    if (this.running)
        throw "Can't start ThrustaSubscriber, it's already started.";
...
ThrustaSubscriber.prototype.stop = function () {
    if (!this.running)
        throw "Can't stop ThrustaSubscriber, it's not running.";
...
ThrustaSubscriber.prototype.SubscriberClass = {
    "websocket": (function () {
